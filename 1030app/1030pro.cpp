                               ...
                               uint size)
{
    ...
    for(i = 0; i < framenum; i++)
    {
        ...
        if(sendframe.canframework.func_5 == CMD_SEND_CFGPARAM &&
                                         sendframe.canframework.devaddr_8 != cf_endnum)
            sendframe.canframework.next_1 = 1;
        ...
        if ( i || (sendframe.canframework.func_5 == CMD_SEND_CFGPARAM &&
                  sendframe.canframework.devaddr_8 != 1 ) )
    ...
    switch(func)
    {
       ...
       case CMD_SEND_CFGPARAM:
       {
        ...
        case CMD_SEND_HEARTCHECK:
        {
            ...
            if(nconfig_map.val(0).para.id == dest)
                heartcout = 0;
            pro_p->can_protocol_send(sendmiddata);
         }
         break;
    ...
    if ( csrxcanp->csstate == CS_CAN_RESET )
    {
        if ( ( csrxcanp->cszjorder[ branch ] + 1 ) != get_dev_addr && csrxcanp->csdevtyle == 0 )
        {
            zprintf1( "mac MAC_ORDER_ERR, csmacorder = %d cszjorder =  %d, %d, %d, \
                        ...
                        csrxcanp->csmacorder, csrxcanp->cszjorder[0],   \
                        csrxcanp->cszjorder[1],csrxcanp->cszjorder[2],  \
        ...
        if ( csrxcanp->csmacstate == MAC_NORMAL )
        {
            ...
            if ( get_zj_index )
            {
                ...
                csrxcanp->state_info.set_dev_type( get_cs_index - 1, csrxcanp->cszjorder[ get_cs_index - 1 ], rxmeg.Data[4]);
            }
            else
            {
            ...
            if ( ( devtype == CS_DEV ) || ( devtype == TK236_IOModule_Salve )      ||   \
                 ( devtype == DEV_256_IO_LOCK ) || ( devtype == DEV_256_IO_PHONE ) ||   \
                 ( devtype == TERMINAL ) )
            {
                csrxcanp->csioorder++;
                if ( get_zj_index )
                {
                    ...
                    csrxcanp->state_info.set_dev_ID( branch, csrxcanp->cszjorder[ get_cs_index - 1 ], slave_io_num_tmp % 0x100);
                }
                else
                {
                ...
                if ( csrxcanp->is_have_config_slaveio( get_zj_index, get_cs_index, slave_io_num_tmp, config_devid ) )          //存在该设备的配置
                {
                    ...
                    if ( csrxcanp->nconfig_map.val( config_devid ).para.type != devtype )
                    {
                        if ( csrxcanp->nconfig_map.val( config_devid ).para.name.size() == 0 )
                        {
                            ...
                            {
                                memcpy( &csrxcanp->nconfig_map.val(config_devid).cscrc, &rxmeg.Data[6], sizeof( csrxcanp->nconfig_map.val(config_devid).cscrc ) );
                                memcpy(csrxcanp->nconfig_map.val(config_devid).csmac, rxmeg.Data, 6);
                            }
                        }
                        else
                        {
                            //printf("compare data = %s\r\n",csrxcanp->nconfig_map.val( config_devid ).para.name.c_str());
                            //printf("compare = %d\r\n",csrxcanp->nconfig_map.val( config_devid ).para.name.find("Module"));
                            switch ( devtype )
                            {
                            ...
                            case DEV_256_IO_PHONE:
                                ...
                                {
                                    csrxcanp->nconfig_map.val( config_devid ).para.type = devtype;
                                    memcpy( &csrxcanp->nconfig_map.val(config_devid).cscrc, &rxmeg.Data[6], sizeof( csrxcanp->nconfig_map.val(devid).cscrc ) );
                                    memcpy(csrxcanp->nconfig_map.val(config_devid).csmac, rxmeg.Data, 6);
                                }
                                else
                                {
...
int configdata_ack_proc(void * pro1030, CANDATAFORM  rxmeg)
{
    ...
    if(csrxcanp->is_have_dev(rxmidframe.canframework.zjaddr_3, rxmidframe.canframework.csaddr_2, rxmidframe.canframework.devaddr_8, soureid) == FALSE) return -2;
...
//    soureid = rxmidframe.canframework.devaddr_8;
    csrxcanp->is_have_dev(rxmidframe.canframework.zjaddr_3, rxmidframe.canframework.csaddr_2, rxmidframe.canframework.devaddr_8, soureid);
    checkframe = rxmidframe;
...
void read_reg_entry( cs_can *csrxcanp, CANDATAFORM  rxmeg )
{
    ...
    if (csrxcanp->is_have_mac_dev(rxmidframe.canframework.zjaddr_3, rxmidframe.canframework.csaddr_2, rxmidframe.canframework.devaddr_8, sourceid))
    {
        ...
        else
        {
            if ( rxmidframe.canframework.ttl_7 == 0 )
            {
                if ( rxmeg.DLC == 4 )
                {
                    memcpy( &( csrxcanp->nconfig_map.val( sourceid ).recv_reg_begin ), rxmeg.Data, sizeof( csrxcanp->nconfig_map.val( sourceid ).recv_reg_begin ) );
                    memcpy( &( csrxcanp->nconfig_map.val( sourceid ).recv_reg_offset ), rxmeg.Data + 2, sizeof( csrxcanp->nconfig_map.val( sourceid ).recv_reg_offset ) );
                }
            }
            else
            {
...
int output_controlack_overproc(void * pro1030, CANDATAFORM  overmeg)
{
    ...
    csrxcanp->is_have_dev(rxmidframe.canframework.zjaddr_3, rxmidframe.canframework.csaddr_2, rxmidframe.canframework.devaddr_8, order);
...
int stop_report_frameproc(void * pro1030, CANDATAFORM  rxmeg)
{
    ...
    //待添加急停处理
    csrxcanp->cs_send_data(CMD_SEND_STOPREPORT_ACK,rxmidframe.canframework.zjaddr_3, rxmidframe.canframework.csaddr_2, rxmidframe.canframework.devaddr_8,rxmeg.Data, 1);
    return 1;
}
...
int auto_report_frameproc(void * pro1030, CANDATAFORM  rxmeg)
{
    ...
    if ( rxmeg.Data[ 0 ] == DEV_256_PHONE || rxmeg.Data[ 0 ] == DEV_256_LOCK || \
         rxmeg.Data[ 0 ] == CS_DEV    || rxmeg.Data[ 0 ] == DEV_256_RELAY )
    {
    ...
    if ( ( dev_function >> BS_FUNCTION & 0x01 ) == 0x01 && rxdata.rxinattr.innum5 == 0x1f )
    {
        ...
        {
            csrxcanp->data_p->set_share_data_value(csrxcanp->slave_io_num[branch] - 1, rxmidframe.canframework.zjaddr_3 ? rxmidframe.canframework.csaddr_2 - 1 : 0,  rxmidframe.canframework.devaddr_8, rxdata.rxinattr.instate2 & 0x01);
        }

        csrxcanp->state_info.set_bs_state(branch, rxmidframe.canframework.devaddr_8-1,  rxdata.rxinattr.instate2 & 0x01);
        ...
        if ( csrxcanp->is_have_dev( zjnum, csnum, soureid, soureid ) )
        {
            ...
            if ( memcmp( rxmeg.Data, &csrxcanp->nconfig_map.val( soureid ).para.type, 1 ) == 0 )
            {
                ...
                if ( rxdata.rxinattr.innum5 != 0x1f )
                {
                    ...
                    {
                        inpara.paraval = csrxcanp->nconfig_map.val(soureid).config_p[INNODEINFO(rxdata.rxinattr.innum5 -1)];
                        if ( rxdata.rxinattr.instyle1 != inpara.inval.instyle1 )
                        {
                            zprintf1("%d auto in error, %d, %d, %d\n", soureid, inpara.inval.inenable1, rxdata.rxinattr.instyle1, inpara.inval.instyle1);
                        }
                        else
                        {
                           if ( rxdata.rxinattr.instyle1 == 0 )
                           {
                               csrxcanp->data_p->set_share_data_value(csrxcanp->nconfig_map.val(soureid).slaveio_order - 1, /*rxmidframe.canframework.csaddr_2*/0,  rxdata.rxinattr.innum5,rxdata.rxinattr.instate2);
                           }
                           else
                           {
                               if(rxmeg.DLC < 5)
                               ...
                               gBugState = 1;
                               {
                                   ...
                                   csrxcanp->data_p->set_share_data_value(csrxcanp->nconfig_map.val(soureid).slaveio_order - 1, /*rxmidframe.canframework.csaddr_2*/0,  rxdata.rxinattr.innum5, devvalue);
                               }
                               gBugState = 2;
                           }
                        }
...
//    csrxcanp->nconfig_map.val(soureid).errcount = 0;
    ...
    if(rxdata.rxinattr.instyle1 == 0)
    {
        csrxcanp->cs_send_data(CMD_SEND_AUTOREPORT_ACK, rxmidframe.canframework.zjaddr_3, rxmidframe.canframework.csaddr_2, rxmidframe.canframework.devaddr_8,rxmeg.Data, rxmeg.DLC);
    }
    else
    {
...
int dev_work_check_callback(void * pro1030, CANDATAFORM  rxmeg)
{
    ...
    if ( csrxcanp->csstate == CS_CAN_NORMAL )//line out,and you need send 03 find line out location
    {
        ...
        else
        {
            ...
            {
                csrxcanp->data_p->set_share_data_value(csrxcanp->slave_io_num[branch] - 1, get_zj_index ? get_cs_index - 1 : 0, get_dev_addr, rxmeg.Data[ 3 ] & 0x01);
    ...
    if ( csrxcanp->csstate == CS_CAN_NORMAL )
    {
        ...
        for ( int i = 0; i < BRANCH_ALL; i++ )
        {
            if ( csrxcanp->cszjorder[i] != csrxcanp->cszjorder_temp[i] )
            {
                ...
                if ( csrxcanp->cszjorder[i] <= csrxcanp->cszjorder_temp[i] ) { //数量增加
                    csrxcanp->nconfig_map.val(0).dev_send_meg(LOW_NUM_MEG, csrxcanp->low_num, sizeof(csrxcanp->low_num));
                } else { //数量减少
...
//                            memcpy(csrxcanp->slave_io_num, csrxcanp->slave_io_num_temp, sizeof(csrxcanp->slave_io_num));
                        }
                    }
                    csrxcanp->nconfig_map.val(0).dev_send_meg(LOW_NUM_LOST, csrxcanp->low_num, sizeof(csrxcanp->low_num));
        ...
        for (int i = 0; i < BRANCH_ALL; i++)
        {
            ...
            csrxcanp->state_info.set_slaveio_num(i, csrxcanp->slave_io_num_temp[ i ] - csrxcanp->mac_cszd_have.get_order_data( i ).mac_cs_have );
        ...
        else if(devtyle != CS_DEV)                  //终端数据
        {
            ...
            for (uint8_t i = 0; i < rxmeg.DLC; i++)
            {
                for (uint8_t j = 0; j < 8; j++)
                {
                    ...
                    //TODO: define 设备在线
                    (csrxcanp->state_info.share_state.data+branch)->devc_state[(ttl-1)*64 + i*8 + j].Dev_State |= 1;
                    if (rxmeg.Data[i] & (0x01<<j))
                    {
                        zprintf1("heart dev %d is lock \r\n", (ttl-1)*64 + i*8 + j);
                        (csrxcanp->state_info.share_state.data+branch)->devc_state[(ttl-1)*64 + i*8 + j].Dev_State |= 4;
                    }
                    else
                    {
                        (csrxcanp->state_info.share_state.data+branch)->devc_state[(ttl-1)*64 + i*8 + j].Dev_State &= ~4;
            ...
            memcpy(&(csrxcanp->state_info.share_state.data+branch)->line_state.BS_Buttion[(ttl-1)*8], rxmeg.Data, rxmeg.DLC);
        }
        else
        {
...
{
    return csrxcanp->nconfig_map.val(devnum).framark.to_ulong() ==
                       ((0x0001 << (ttl)) -1);
}
bool is_heartframe_correct(cs_can *csrxcanp, uint8_t devnum, int ttl, CANDATAFORM  rxmeg, uint16_t devtyle)
{
    ...
    {
//            printf("heartframe_correct_error %d, %d!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n",DEVPOLLSIZE(devtyle), ((ttl *8)+rxmeg.DLC) );
       ret = TRUE;
    }
...
void heart_nonextprocess(CANPRODATA *rxprodata,cs_can *csrxcanp, uint8_t devnum, int ttl, CANDATAFORM  rxmeg)
{
    ...
    else
    {
        ...
        else if(devtyle != CS_DEV)                                          //终端
        {
            ...
            {
                csrxcanp->data_p->set_share_data_value(bsnum - 1, rxmidframe.canframework.zjaddr_3 ? rxmidframe.canframework.csaddr_2 - 1 : 0,  i+1, ((csrxcanp->state_info.share_state.data+branch)->devc_state[i].Dev_State & 0x04)>>2);
            }
        }
    }
}
...
int max_heart_ackframe_proc(void * pro1030, CANDATAFORM  rxmeg)
{
    ...
    if ( rxprodata != NULL )
    {
        ...
        else
        {
            ...
            if ( csrxcanp->is_have_config_dev( csrxcanp->heartcout + 1, soureid ) && csrxcanp->nconfig_map.val(soureid).para.type == TERMINAL )
            {
...
int err_reportframe_proc(void * pro1030, CANDATAFORM  rxmeg)
{
    ...
    if(csrxcanp == NULL) return -1;
    csrxcanp->cs_send_data(CMD_SEND_ERRREPORT_ACK, rxmidframe.canframework.zjaddr_3, rxmidframe.canframework.csaddr_2, rxmidframe.canframework.devaddr_8,rxmeg.Data, rxmeg.DLC);
...
void cs_can::set_dev_enable()
{
    ...
    for (uint8_t k = 0; k < slave_io_num[2]; k++)
    {
        ...
        {
            state_info.set_dev_enable_state(nconfig_map.val(order).para.id, nconfig_map.val(order).devenable);
            zprintf3("state_info.set_dev_enable_state(%d, %d)\n",nconfig_map.val(order).para.id,nconfig_map.val(order).devenable);
        }
    }
}
...
//                cs_pro_p->nconfig_map.val(0).dev_send_meg(CS_REST_END_MEG, reset_msg, sizeof(reset_msg));
                sleep(3);
            }
        }
...
//    nconfig_map.val(0).dev_send_meg(CS_REST_END_MEG, reset_msg, sizeof(reset_msg));
    uint8_t bs_data[2] = {0};
    ...
    {
        low_num[0] = 0;
        low_num[1] = slave_io_num[ i ] - mac_cszd_have.get_order_data( i ).mac_terminal_have - mac_cszd_have.get_order_data( i ).mac_cs_have;
...
int dev1030_output(void * midp, soutDataUnit val)
{
    ...
    else
    {
        ...
        if(pro->get_dev_type(order) == CS_DEV ||
             pro->get_dev_type(order) ==  TERMINAL)
        {
        ...
        if(pro->nconfig_map.val(order).get_outnode_tyle(val.pointid) == 0)
        {
            ...
            if (val.childid)
            {
                pro->cs_send_data(CMD_SEND_SWOUTPUT, 1, val.childid+1, pro->nconfig_map.val(order).dev_off, outvalue, 1);
            }
            else
            {

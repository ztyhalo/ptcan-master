...
int mac_inquire_ack_proc(void *pro1030, CANDATAFORM rxmeg)
{
    ...
    if (csrxcanp->csstate == CS_CAN_RESET)
    {
        ...
        if (csrxcanp->csmacstate == MAC_NORMAL)
        {
            ...
            if ((devtype == CS_DEV) || (devtype == TK236_IOModule_Salve) || (devtype == DEV_256_IO_LOCK) || (devtype == DEV_256_IO_PHONE) || (devtype == TERMINAL))
            {
                csrxcanp->csioorder++;
                if (get_zj_index)
                {
...
int dev_break_check_callback(void *pro1030, CANDATAFORM rxmeg)
{
    ...
    int dev_work_check_callback(void *pro1030, CANDATAFORM rxmeg)
    {
        ...
        {
            if (((csrxcanp->cszjorder_temp[branch] + 1) != get_dev_addr && csrxcanp->csdevtyle == 0) || csrxcanp->reset_state == DEV_RESET_ING)
            {
                csrxcanp->csmacstate = MAC_ORDER_ERR;
                ...
                if (rxprodata != NULL)
                {
                    ...
                    else
                    {
                        ...
                        else if (csrxcanp->csdevtyle == 0)    // 分次发送查询帧
                        {
                            ...
                            {
                                csrxcanp->heartcout++;
                            } while (csrxcanp->is_have_check_dev(csrxcanp->heartcout + 1, soureid) && (csrxcanp->ndev_map.val(soureid).type != CS_DEV) && (csrxcanp->ndev_map.val(soureid).type != TK236_IOModule_Salve) && (csrxcanp->ndev_map.val(soureid).type != DEV_256_IO_PHONE) && (csrxcanp->ndev_map.val(soureid).type != DEV_256_IO_LOCK) && (csrxcanp->ndev_map.val(soureid).type != TERMINAL));
            ...
            uint8_t cs_can::get_bs_mac_num(uint8_t branch)
            {
                ...
                {
                    if ((ndev_map.val(i).type == DEV_256_PHONE) || (ndev_map.val(i).type == DEV_256_LOCK) || (ndev_map.val(i).type == DEV_256_IO_PHONE) || (ndev_map.val(i).type == DEV_256_IO_LOCK) || (ndev_map.val(i).type == CS_DEV) || (ndev_map.val(i).type == DEV_256_RELAY))
                    {
                        bsnum++;
                    }
                }

#ifndef __256COMMON_H__
#define __256COMMON_H__
...
#define CS256_INDATA_SIZE  1
#define CS256_OUTDATA_SIZE 1
#define CS256_POLL_HEAD    1    // 16bit为单位

#define CS256_CONFIG_BUF 1


#define CS256REG_BUF_MAX (CSDEV_INNODE_NUM * CS256_INDATA_SIZE + CSDEV_OUTNODE_NUM * CS256_OUTDATA_SIZE)
...
class Max_256State_Pro;


class CAN_DEV_APP_INFO_256
{
public:
    uint16_t  dev_off;
    ...
    Max_256State_Pro        *stateinfo;
    MsgMng                  *msgmng_p;
    Pt_Devs_ShareData       *devdata_p;
public:
    CAN_DEV_APP_INFO_256()
    {
        memset(this, 0x00, sizeof(CAN_DEV_APP_INFO_256));
    }
    ~CAN_DEV_APP_INFO_256()
    {
        if(config_p != NULL)
...
class CAN_256DEV_APP:public CAN_DEV_APP_INFO_256
{
public:
...
public:
    ...
    int set_config_head(void)
    {
        ...
        iodata     = new uint16_t[para.innum * CS256_INDATA_SIZE + para.outnum * CS256_OUTDATA_SIZE];
    ...
    {
        configsize = CS256_CONFIG_BUF + CONF_HEAD;
        config_p   = new uint16_t[configsize];
    ...
    int get_outnode_tyle(int node)    // 点从1开始排序
    {
        ...
        return outparam.out256conf.outstyle1;
    }
    ...
    uint16_t get_input_data(int node)    // 点从1开始排序
    {
        ...
        {
            IOPUTDATA ioputdata;
            ioputdata.iovalue = iodata[(node - 1) * CS256_INDATA_SIZE];
    ...
    uint16_t get_output_data(int node)    // 点从1开始排序
    {
        ...
        {
            IOPUTDATA ioputdata;
            ioputdata.iovalue = iodata[para.innum * CS256_INDATA_SIZE + (node - 1) * CS256_OUTDATA_SIZE];
...
class Max_256State_Pro : public Dev_Map_T< char >
{
public:
    int                             cs_have;
    uint8_t                         tatol_dev;
...
public:
    ...
    ~Max_256State_Pro()
    {
        zprintf3("destory Max_256State_Pro!\n");
    }

#ifndef __256_PRO_C__
#define __256_PRO_C__
...
{
    cs_can256_info::CANFRAMEID rxmidframe;
 ...
 * 功能：cs读取配置数据并设置配置信息
 ***********************************************************************************/
int cs_can256::add_default_dev(int zjnum, int csnum, int devid, int io_num, uint16_t type)
{
    CAN_256DEV_APP mid;
    int         order;
    int         dev_off = devid - 1;
    ...
    else
    {
        ...
        nconfig_map.insert(pair< int, CAN_256DEV_APP >(cf_devnum, mid));
...
int cs_can256::pt_configdata_set(CAN_DEV_INFO &dev, uint8_t cs_num, uint8_t dev_num)
{
    CAN_256DEV_APP mid;
 ...
 *
 ***********************************************************************************/
int cs_can256::pt_configdata_set(CAN_DEV_INFO &dev, uint16_t dev_off)
{
    CAN_256DEV_APP mid;
...
int cs_can256::get_dev_id(int order)
{
    CAN_256DEV_APP *mid = NULL;
...
int cs_can256::get_dev_off(int devid)
{
 ...
 * 功能：根据发送设置发送不同的信息
 ***********************************************************************************/
void cs_can256::send_data_ttlproc(cs_can256_info::CANFRAMEID &sendframe, CANDATAFORM &senddata, uint8_t *data, uint size)
{
    uint i;
    uint framenum;
    framenum = size / 8;
    ...
    for (i = 0; i < framenum; i++)
    {
        ...
        if (sendframe.canframework.func_5 == cs_can256_info::CMD_SEND_CFGPARAM && sendframe.canframework.devaddr_8 != cf_endnum)
            sendframe.canframework.next_1 = 1;
        ...
        if (i || (sendframe.canframework.func_5 == cs_can256_info::CMD_SEND_CFGPARAM && sendframe.canframework.devaddr_8 != 1))
 ...
 * 功能：cs应用发送的协议帧
 ***********************************************************************************/
void cs_can256::cs_send_data(uint func, int zjnum, int csnum, int dest, uint8_t *data, uint size)
{
    cs_can256_info::CANFRAMEID  sendmidframe;
    ...
    memset(&sendmidframe, 0, sizeof(cs_can256_info::CANFRAMEID));
    ...
    switch (func)
    {
        ...
        case 1:
            break;
        case cs_can256_info::CMD_SEND_MASTERREST:
            ...
            break;
        case cs_can256_info::CMD_SEND_MACCHECK:
            sendmidframe.canframework.devaddr_8 = 255;
            sendmidframe.canframework.func_5    = func;
            sendmidframe.canframework.csaddr_2  = csnum;
            sendmidframe.canframework.zjaddr_3  = zjnum;
            ...
            pro_p->can_protocol_send(sendmiddata);
            break;
        case cs_can256_info::CMD_SEND_PARACORRECT:
            ...
            break;
        case cs_can256_info::CMD_SEND_CFGPARAM:
        {
        ...
        break;
        case 12:
        case cs_can256_info::CMD_SEND_ERRREPORT_ACK:
            sendmidframe.canframework.devaddr_8 = dest;
            sendmidframe.canframework.func_5    = func;
            sendmidframe.canframework.csaddr_2  = csnum;
            sendmidframe.canframework.zjaddr_3  = zjnum;
            ...
            pro_p->candrip->write_send_data(sendmiddata);
            break;
        case cs_can256_info::CMD_SEND_AUTOREPORT_ACK:
        case cs_can256_info::CMD_SEND_BROKENTAIL:
            sendmidframe.canframework.devaddr_8 = dest;
            sendmidframe.canframework.func_5    = func;
            sendmidframe.canframework.csaddr_2  = csnum;
            sendmidframe.canframework.zjaddr_3  = zjnum;
        ...
        case 15:
            ...
            pro_p->can_protocol_send(sendmiddata);
            break;
        case cs_can256_info::CMD_REV_LOCK_LOCATION_ACK:
            sendmidframe.canframework.devaddr_8 = dest;
            sendmidframe.canframework.func_5    = func;
            sendmidframe.canframework.csaddr_2  = csnum;
            sendmidframe.canframework.zjaddr_3  = zjnum;
            ...
            break;
        case cs_can256_info::CMD_SEND_READFUNC_HIGH:
        {
            CANPROHEAD *midinfop = NULL;
        ...
        break;
        case cs_can256_info::CMD_SEND_FOUTPUT:
        case cs_can256_info::CMD_SEND_SWOUTPUT:
        case 21:
        case 23:
        {
            CANPROHEAD *midinfop = NULL;
        ...
        break;
            //        case cs_can256_info::CMD_SEND_WORK_DEV_CHECK:
            //            sendmidframe.canframework.devaddr_8 = 255;
            //            sendmidframe.canframework.func_5    = func;
            //            sendmidframe.canframework.csaddr_2  = csnum;
            //            sendmidframe.canframework.zjaddr_3  = zjnum;
            ...
            //            pro_p->can_protocol_send(sendmiddata);
            //            break;
        case cs_can256_info::CMD_SEND_BREAK_CHECK:
            sendmidframe.canframework.devaddr_8 = 255;
            sendmidframe.canframework.func_5    = func;
            sendmidframe.canframework.csaddr_2  = csnum;
            sendmidframe.canframework.zjaddr_3  = zjnum;
            ...
            pro_p->can_protocol_send(sendmiddata);
            break;
        case cs_can256_info::CMD_SEND_HEARTCHECK:
        {
            sendmidframe.canframework.devaddr_8 = dest;
            sendmidframe.canframework.func_5    = func;
            sendmidframe.canframework.csaddr_2  = csnum;
 ...
 * 功能：该设备是否存在
 ***********************************************************************************/
bool cs_can256::is_have_dev(int zjnum, int csnum, int devid, int &order)
{
    // devnum
    if (zjnum)
    {
...
bool cs_can256::is_have_config_slaveio(int zjnum, int csnum, int slaveio_order, int &order)
{
    if (zjnum)
    {
 ...
 * 功能：该设备是否存在
 ***********************************************************************************/
bool cs_can256::is_have_mac_dev(int paraid, int &order)
{
    if (ndev_map.size( ) == 0)
    {
...
bool cs_can256::is_have_mac_dev(int zjnum, int csnum, int devid, int &order)
{
    if (zjnum)
    {
        devid |= 0x100 << (csnum - 2);
 ...
 * 功能：该设备是否存在
 ***********************************************************************************/
bool cs_can256::is_have_check_dev(int devid, int &order)
{
 ...
 * 功能：该设备是否存在
 ***********************************************************************************/
bool cs_can256::is_have_config_dev(int devid, int &order)
{
 ...
 * 功能：该设备是否存在
 ***********************************************************************************/
CS_BRANCH_TYPE cs_can256::get_branch(cs_can256_info::CANFRAMEID rxmidframe)
{
...
{
    cs_can256 *csrxcanp = static_cast<cs_can256 *>(pro1030);
    Q_UNUSED(overmeg);
    if (csrxcanp == NULL)
    {
        return 0;
...
static int mac_inquire_ack_proc(void *pro1030, CANDATAFORM rxmeg)
{
    ...
    CS_BRANCH_TYPE branch;
    cs_can256_info::CANFRAMEID     rxmidframe, checkframe;
    ...
    else if (csrxcanp->csstate == CS_CAN_NORMAL)
    {
        ...
        if (devtype == TERMINAL_256)
        {
            ...
            csrxcanp->cs_send_data(cs_can256_info::CMD_SEND_PARACORRECT, get_zj_index, get_cs_index, get_dev_addr, NULL, 0);
        }
    }
#endif
    return 0;
...
static int enter_workmode_frameproc(void *pro1030, CANDATAFORM rxmeg)
{
    Q_UNUSED(pro1030);
    Q_UNUSED(rxmeg);
    // printf("!!!!!!!!!!!!!!!!enter work~~~~~~~~~~~~~~~~~~~~\r\n");
...
static int configdata_ack_proc(void *pro1030, CANDATAFORM rxmeg)
{
    int        soureid;
    cs_can256_info::CANFRAMEID rxmidframe;
    cs_can256_info::CANFRAMEID checkframe;
    cs_can256    *csrxcanp = static_cast<cs_can256 *>(pro1030);
    ...
    if (csrxcanp->nconfig_map.val(soureid).para.id == csrxcanp->cf_endnum)
    {
        ...
        csrxcanp->cs_send_data(cs_can256_info::CMD_SEND_PARACORRECT, 0x07, 0, 255, NULL, 0);
 ...
 * 功能：配置数据超时处理           //有问题
 ***********************************************************************************/
static int configdata_overtimeproc(void *pro1030, CANDATAFORM rxmeg)
{
    (void)rxmeg;
    cs_can256    *csrxcanp = static_cast<cs_can256 *>(pro1030);
    // cs_can256_info::CANFRAMEID rxmidframe;
 ...
 * 功能：读寄存器应答帧处理
 ***********************************************************************************/
static void read_reg_entry(cs_can256 *csrxcanp, CANDATAFORM rxmeg)
{
    // uint16_t   devtyle = 0;
    uint8_t    branch;
    int        sourceid;
 ...
 * 功能：读寄存器应答帧处理
 ***********************************************************************************/
static int readreg_callback_frameproc(void *pro1030, CANDATAFORM rxmeg)
{
    cs_can256 *csrxcanp = static_cast<cs_can256 *>(pro1030);
...
//     // 待添加急停处理
//     csrxcanp->cs_send_data(cs_can256_info::CMD_SEND_STOPREPORT_ACK, rxmidframe.canframework.zjaddr_3, rxmidframe.canframework.csaddr_2,
...
static int max_stop_report_frameproc(void *pro1030, CANDATAFORM rxmeg)
{
    ...
    csrxcanp->cs_send_data(cs_can256_info::CMD_SEND_STOPREPORT_ACK, zjnum, csnum, soureid, rxmeg.Data, 1);
 ...
 * 功能：段路位置帧处理
 ***********************************************************************************/
static int break_location_report_frameproc(void *pro1030, CANDATAFORM rxmeg)
{
    cs_can256_info::CANFRAMEID rxmidframe;
    cs_can256    *csrxcanp = static_cast<cs_can256 *>(pro1030);
    int        branch;
    ...
    if (rxmidframe.canframework.func_5 == cs_can256_info::CMD_SEND_LOCK_LOCATION)
    {
        uint8_t offset = 0;
        uint8_t cmd;
        uint8_t location;
        ...
        csrxcanp->cs_send_data(cs_can256_info::CMD_REV_LOCK_LOCATION_ACK, rxmidframe.canframework.zjaddr_3,
...
ACK_PROSS:
    ...
    if (rxdata.rxinattr.instyle1 == 0)
    {
        csrxcanp->cs_send_data(cs_can256_info::CMD_SEND_AUTOREPORT_ACK, rxmidframe.canframework.zjaddr_3,
 ...
 * 功能：断线查询帧接收回调
 ***********************************************************************************/
static int dev_break_check_callback(void *pro1030, CANDATAFORM rxmeg)
{
    cs_can256        *csrxcanp = static_cast<cs_can256 *>(pro1030);
    CS_BRANCH_TYPE branch;
    cs_can256_info::CANFRAMEID     rxmidframe;
    ...
    if ((csrxcanp->cszjorder_temp[branch] + 1) != get_dev_addr || csrxcanp->csstate != CS_CAN_NORMAL || csrxcanp->reset_state == cs_can256_info::DEV_RESET_ING)    // 号不连续
    {
 ...
 * 功能：断线查询帧接收超时
 ***********************************************************************************/
static int dev_break_check_overtime(void *pro1030, CANDATAFORM rxmeg)
{
    (void)rxmeg;
    cs_can256        *csrxcanp            = static_cast<cs_can256 *>(pro1030);
    ...
    if (IS_ONE(csrxcanp->break_send_status, 0))
    {
        if (csrxcanp->csstate == CS_CAN_NORMAL)
        {
            if (csrxcanp->reset_state == cs_can256_info::DEV_RESET_OVER && csrxcanp->reset_msg[0] == RESET_SUCCESS)
            {
                csrxcanp->nconfig_map.val(0).dev_send_meg(
 ...
 * 功能：电压电流主动上报
 ***********************************************************************************/
// static int receive_a_v_callback(void *pro1030, CANDATAFORM rxmeg)
// {
//     cs_can256    *csrxcanp = static_cast<cs_can256 *>(pro1030);
//     uint8_t    branch;
//     int        sourceid;
 ...
 * 功能：心跳应答帧处理函数调用的子函数
 ***********************************************************************************/
static void heart_frame_over(CANPRODATA *rxprodata, cs_can256 *csrxcanp)
{
...
// static __inline__ void tk100_data_pro(cs_can256 *csrxcanp, CANDATAFORM rxmeg)
// {
...
static void com_heart_nextprocess(CANPRODATA *rxprodata, cs_can256 *csrxcanp, uint8_t devnum, int ttl, CANDATAFORM rxmeg)
{
    uint16_t       devtyle = 0;
    CS_BRANCH_TYPE branch;
    cs_can256_info::CANFRAMEID     rxmidframe;
...
static __inline__ bool count_heart_nextprocess(cs_can256 *csrxcanp, uint8_t devnum, uint ttl)
{
    return csrxcanp->framark[devnum].to_ulong( ) == ((0x0001U << (ttl)) - 1U);
}

static bool is_heartframe_correct(cs_can256 *csrxcanp, uint8_t devnum, int ttl, CANDATAFORM rxmeg, uint16_t devtyle)
{
    bool ret = FALSE;
    Q_UNUSED(rxmeg);
    Q_UNUSED(devtyle);
...
static void heart_nonextprocess(CANPRODATA *rxprodata, cs_can256 *csrxcanp, uint8_t devnum, int ttl, CANDATAFORM rxmeg)
{
    uint16_t   devtyle = 0;
    cs_can256_info::CANFRAMEID rxmidframe;
    int        branch;
...
static int err_reportframe_proc(void *pro1030, CANDATAFORM rxmeg)
{
    ...
    if (csrxcanp == NULL)
        return -1;
    csrxcanp->cs_send_data(cs_can256_info::CMD_SEND_ERRREPORT_ACK, rxmidframe.canframework.zjaddr_3, rxmidframe.canframework.csaddr_2,
...
static int slavereset_frame_proc(void *pro1030, CANDATAFORM rxmeg)
{
    ...
    if (devtype != TERMINAL_256)
    {
        ...
        else if (csrxcanp->reset_state == cs_can256_info::DEV_RESET_ING)
        {
    ...
    else
    {
        if (csrxcanp->reset_state == cs_can256_info::DEV_RESET_OVER)
        {
            ...
            csrxcanp->cs_send_data(cs_can256_info::CMD_SEND_MACCHECK, 0, 0, 255, NULL, 0);
        }
    }
#endif
    return 0;
...
cs_can256::cs_can256(ncan_protocol *pro, const QString & key, int branch_num, int reset_enable)
{
...
int cs_can256::cs_can_reset_sem(void)
{
    if (init_over == 1)
    {
        if (reset_state != cs_can256_info::DEV_RESET_ING)
        {
            reset_state = cs_can256_info::DEV_RESET_ING;
...
int cs_can256::cs_can_reset_pro(void)
{
    if (auto_reset)
    {
...
uint8_t cs_can256::get_config_low_num(void)
{
    return cf_file_num ? cf_file_num - cf_cs_num[0] - cf_cs_num[1] - cf_cs_num[2] : cf_file_num;
}
uint8_t cs_can256::get_mac_low_num(void)
{
...
uint8_t cs_can256::get_bs_mac_num(uint8_t branch)
{
    uint8_t bsnum = 0;
...
void cs_can256::add_mac_cszd_branch(int zjnum, int csnum)
{
    uint8_t      branch_line;
    CSZD_IS_HAVE tmp;
...
void cs_can256::set_mac_cs_have(int zjnum, int csnum)
{
    uint8_t branch_line;
...
void cs_can256::bs_type_location_report(uint8_t bs_type)
{
    uint8_t data[2];
...
void cs_can256::change_report_bs_location(uint8_t branch, uint8_t bs_location)
{
    if (branch == 0)
    {
...
void cs_can256::bs_type_location_report(uint8_t branch, uint8_t bs_type, uint8_t bs_location)
{
    uint8_t data[2];
...
void cs_can256::bs_type_location_report(uint8_t bs_type, uint8_t bs_location)
{
    uint8_t data[2];
 ...
 * 功能：cs发送配置参数
 ***********************************************************************************/
void cs_can256::max_reset_data(void)
{
 ...
 * 功能：cs发送配置参数
 ***********************************************************************************/
void cs_can256::send_configdata(void)
{
    uint i;
    ...
    for (i = 0; i < configsize; i++)
    {
        if (nconfig_map.val(i).conf_state == CS_CONFIGING)
        {
            ...
            cs_send_data(cs_can256_info::CMD_SEND_CFGPARAM, 0, 0, nconfig_map.val(i).para.id, (uint8_t *)nconfig_map.val(i).config_p,
        ...
        while(1)
        {
            ...
            if(data == dataLast)
            {
                if((logicState == 0) && (++heartErrCnt >= 14))
                {
                    ...
                    (static_cast<cs_can256 *>(para))->cs_can_reset_sem();
                    heartErrCnt = 0;
 ...
 ***********************************************************************************/
void *cs_can256::cs_protocol_init(void *para)
{
    uint8_t reset_reason[4] = { 0, 0, 0, 0 };
    if ((static_cast<cs_can256 *>(para))->cs_init( ) != 0)
    {
        zprintf1("init fail send sem!\n");
        reset_reason[0] = PTCAN_RESET_INIT;
        reset_reason[1] = (static_cast<cs_can256 *>(para))->auto_reset;
        (static_cast<cs_can256 *>(para))->nconfig_map.val(0).dev_send_meg(RESET_REASON, reset_reason, sizeof(reset_reason));
        (static_cast<cs_can256 *>(para))->cs_can_reset_sem( );
    }
    // printf("cs_protocol_init end\r\n");
    (static_cast<cs_can256 *>(para))->reset_state = cs_can256_info::DEV_RESET_OVER;
    return NULL;
}
...
{
    // int     reset_no_err;
    cs_can256 *cs_pro_p     = static_cast<cs_can256 *>(para);
    while (1)
    {
        ...
        cs_pro_p->reset_state = cs_can256_info::DEV_RESET_OVER;
    }
    return NULL;
}

static int mac_send_entry(void *para)
{
    cs_can256 *cs_pro_p                           = (static_cast<cs_can256 *>(para));
...
static int dev_break_send_entry(void *para)
{
    cs_can256 *cs_pro_p = (static_cast<cs_can256 *>(para));
...
void cs_can256::set_dev_status(uint8_t devid, uint8_t state)
{
...
void cs_can256::set_dev_map_state(uint8_t devid, uint8_t state)
{
    if (state)
    {
...
void cs_can256::set_dev_state(uint8_t devid, uint8_t state)    // Start from 1
{
    uint16_t dev_off;
...
{
    Q_UNUSED(arg);
    cs_can256 *cs_param           = static_cast<cs_can256 *>(arg);
    ...
    while (1)
    {
        ...
        if ((cs_param->cs_jt_status != CS_JT_OK && button_press_shake == 2) || cs_param->cut_check_flag == 1)
        {
            ...
            {
                time_delay = 0;
                if (cs_param->reset_state == cs_can256_info::DEV_RESET_OVER)
                {
                    cs_param->cs_send_data(cs_can256_info::CMD_SEND_BREAK_CHECK, 0, 0, 255, NULL, 0);
                }
            }
            time_delay++;
        }
 ...
 * 功能：cs配置初始化函数
 ***********************************************************************************/
int cs_can256::cs_config_init(void)
{
    cs_can256_info::CANFRAMEID overmidframe;
    ...
    overmidframe.canframework.func_5    = cs_can256_info::CMD_SEND_HEARTCHECK;
...
// {
//     poll->father->can_protocol_send(static_cast<cs_can256 *>(poll->para)->pollFrame);
//     return 0;
// }
void cs_can256::reset_all_dev(void)
{
    cs_can256_info::CANFRAMEID  sendmidframe;
    CANDATAFORM sendmiddata;
    ...
    sendmidframe.canframework.devaddr_8 = 255;
    sendmidframe.canframework.func_5    = cs_can256_info::CMD_SEND_MASTERREST;
    sendmidframe.canframework.dir_1     = 1;
...
void cs_can256::add_poll_frame(void)
{
    if (polltimer_id == 0)
    {
 ...
 * 功能：获得设备类型
 ***********************************************************************************/
uint16_t cs_can256::get_dev_type(uint8_t devid)
{
...
void cs_can256::set_devonline_num(uint8_t val)
{
    csmacorder = val;
...
int cs_can256::cs_init(void)
{
    ...
    cs_send_data(cs_can256_info::CMD_SEND_MASTERREST, 0x07, 0, 255, NULL, 0);
    do
    {
        sem_wait(&statechg);
    ...
    cs_send_data(cs_can256_info::CMD_SEND_MACCHECK, 0, 0, 255, NULL, 0);
    do
    {
        sem_wait(&statechg);
    } while (csstate != CS_CAN_MACREQ);
    reset_state = cs_can256_info::DEV_RESET_CONFIG;
    ...
    else
    {
        ...
        cs_send_data(cs_can256_info::CMD_SEND_PARACORRECT, 0x07, 0, 255, NULL, 0);
    ...
    cs_send_data(cs_can256_info::CMD_SEND_HEARTCHECK, 0, 0, 0xFF, NULL, 0);
...
/***********************************************************************************
 * 函数名：int dev256_output(void * midp, sDataUnit val)
 * 功能：cs应用层控制输出函数
 *
 ***********************************************************************************/
int dev256_output(void *midp, soutDataUnit val)
{
    cs_can256 * pro = static_cast<cs_can256 *>(midp);
    ...
    if (val.pointid == cs_can256_info::CMD_SEND_READFUNC_HIGH)
    {
        ...
        zprintf1("read reg \r\n");
        pro->cs_send_data(cs_can256_info::CMD_SEND_READFUNC_HIGH, 0, 0, val.parentid, can_data, 4);
    }
    else
    {
        struct timeval tv;
        ...
        if (pro->reset_state == cs_can256_info::DEV_RESET_ING)
        {
        ...
        if (pro->nconfig_map.val(order).get_outnode_tyle(val.pointid) == 0)
        {
            ...
            {
                pro->cs_send_data(
                    cs_can256_info::CMD_SEND_SWOUTPUT, 1, val.childid + 1, pro->nconfig_map.val(order).dev_off, outvalue, 1);
            }
            else
            {
                pro->cs_send_data(cs_can256_info::CMD_SEND_SWOUTPUT, 0, 0, pro->nconfig_map.val(order).dev_off, outvalue, 1);
            }
        }
        else
        {
            ...
            if (val.childid)
            {
                ...
                pro->cs_send_data(cs_can256_info::CMD_SEND_FOUTPUT, 1, val.childid + 1, pro->nconfig_map.val(order).dev_off, outvalue, 3);
            }
            else
            {
                pro->cs_send_data(cs_can256_info::CMD_SEND_FOUTPUT, 0, 0, pro->nconfig_map.val(order).dev_off, outvalue, 3);
            }
        }
    }
    return 0;

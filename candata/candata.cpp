...
{
    if(dev > readxml.dev[0].child.size()) return -1;
    CAN_DEV_INFO middev = readxml.dev[0].child[dev-1];

    if(child > 0)
    {
        if(child > middev.child.size()) return -2;
        if(innode > middev.child[child-1].inode.size()) return -3;
        val = middev.child[child-1].inode[innode-1];
    }
    else
    {
        if(innode > middev.inode.size()) return -3;

...
void PRINTF_CLASS::timeprintf(const char* format, ...)
{
    ...
    if (pfd != NULL)
    {
        ...
        fprintf(pfd, "%d-%02d-%02d %02d:%03d:%02d ", 1900 + p->tm_year, 1 + p->tm_mon, p->tm_mday, p->tm_hour,
            p->tm_min, p->tm_sec);
        vfprintf(pfd, format, args);
        va_end(args);
        fflush(pfd);
...
void PRINTF_CLASS::timemsprintf(const char* format, ...)
{
    ...
    if (pfd != NULL)
    {
        ...
        fprintf(pfd, "%d-%02d-%02d %02d:%02d:%02d.%06ld ", 1900 + p->tm_year, 1 + p->tm_mon, p->tm_mday, p->tm_hour,
            p->tm_min, p->tm_sec, tv.tv_usec);
        vfprintf(pfd, format, args);
        va_end(args);
        fflush(pfd);
    }
